...
// Anniversary countdown to October 10
(function() {
  var el = document.getElementById('anniversaryCount');
  if (!el) return;
  var now = new Date();
  var target = new Date(now.getFullYear(), 9, 10);
  if (now > target) target = new Date(now.getFullYear() + 1, 9, 10);
